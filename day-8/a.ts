...
export async function day8a(data: string[]) {
  ...
  antennaPairs.forEach((pairs, freq) => {
    pairs.forEach(([[y1, x1], [y2, x2]]) => {
      // get slope
      // create 2 new antiNodes
      const yd = y2 - y1;
      const xd = x2 - x1;
      const p1B: Pos = [y1 - yd, x1 - xd];
      const p2B: Pos = [y2 + yd, x2 + xd];

      const prevAntiNodes = antiNodes.get(freq) ?? [];
      const newAntiNodes: Pos[] = [];

      if (
        positionInRange(p1B, maxRow, maxCol) &&
      ...
      if (
        positionInRange(p2B, maxRow, maxCol) &&
      ...
      antiNodes.set(freq, [...prevAntiNodes, ...newAntiNodes]);
    });
  });
